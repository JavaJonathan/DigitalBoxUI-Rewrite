...
const TONE_SX = (tone: Tone) => (t: import('@mui/material/styles').Theme) => {
  const map: Record<Tone, { fg: string; bg: string; dot: string }> = {
    neutral: { fg: (t.vars ?? t).palette.text.secondary, bg: (t.vars ?? t).palette.surface.sunken, dot: (t.vars ?? t).palette.text.disabled },
    success: { fg: (t.vars ?? t).palette.success.dark, bg: (t.vars ?? t).palette.success.light, dot: (t.vars ?? t).palette.success.main },
    danger: { fg: (t.vars ?? t).palette.error.dark, bg: (t.vars ?? t).palette.error.light, dot: (t.vars ?? t).palette.error.main },
    warning: { fg: (t.vars ?? t).palette.warning.dark, bg: (t.vars ?? t).palette.warning.light, dot: (t.vars ?? t).palette.warning.main },
    info: { fg: (t.vars ?? t).palette.primary.dark, bg: (t.vars ?? t).palette.primary.light, dot: (t.vars ?? t).palette.primary.main },
  };
  const c = map[tone];
  return {
    display: 'inline-flex',
